...
<table border="1" style="border-collapse: collapse">
    ...
    <tr>
        <td>{{ $data['hotelClassification']}}</td>
    </tr>
    <tr>
        <td>{{ $data['hotel_prefecture'].$data['hotel_city'].$data['hotel_block']}}</td>
    </tr>
    <tr>
        <td>チェックイン/アウト {{ $data['checkin_time']}} ~ {{ $data['checkout_time']}}</td>
    </tr>
    <tr>
        <td>連絡先：{{ $data['hotel_tel']}}</td>
    </tr>
</table>
...
<table border="1" style="border-collapse: collapse">
    ...
    <tr>
        ...
        <td>¥{{$plan->price}}</td>
        <td><button><a href="{{ url('member/add_reservation'.'/'.$plan->id) }}">予約する</a></button></td>
    </tr>
    @endforeach
</table>
@else
